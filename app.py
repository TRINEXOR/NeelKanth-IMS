...
import sqlite3, os, csv, io, json, time, sys, platform, hashlib, secrets, re
...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', secrets.token_hex(32))from flask import Flask, render_template, request, jsonify, send_file, Response, stream_with_context, session, redirect, url_for
import sqlite3, os, csv, io, json, time, sys, platform, hashlib, secrets, re
from datetime import datetime
import openpyxl
from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
...
def is_valid_email(email):
    # Accept any valid email, not just Gmail
...
def api_signup():
    ...
    if not is_gmail(email):
        return jsonify({'error': 'Please sign up with a Gmail account (@gmail.com)'}), 400
    if len(password) < 8:
...
def boot_stream():
    def generate():
        ...
        time.sleep(0.2)
        yield _sse('step', {'msg': 'Loading user session', 'status': 'running'})
        time.sleep(0.25)
        with get_db() as conn:
            user_count = conn.execute('SELECT COUNT(*) FROM users').fetchone()[0]
        yield _sse('step', {'msg': f'Session valid · {user_count} registered user(s)', 'status': 'ok'})
        time.sleep(0.2)
...
if __name__ == '__main__':
    ...
    print(f"   Signup: http://127.0.0.1:{port}/signup")
    print("   Features: Auth · CRUD · Analytics · Export · Alerts")
    print("─" * 52)
